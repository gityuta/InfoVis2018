function Isosurfaces( volume, isovalue, screen, v_type, f_type, color_map_type)
{
    ...
    {   
        ...
        }else if(color_map_type == 'white_blue'){
            var R = Math.max(1 - S, 0.0 );
            var G = Math.max(1 - S, 0.0 );
            var B = Math.cos( 0 );
        }else if(color_map_type == 'white_green'){
            var R = Math.max(1 - S, 0.0 );
            var G = Math.cos( 0 );
            var B = Math.max(1 - S, 0.0 );
        }else if(color_map_type == 'white_red'){
            var R = Math.cos( 0 );
            var G = Math.max(1 - S, 0.0 );
            var B = Math.max(1 - S, 0.0 );
        }
    ...
    var material = new THREE.ShaderMaterial({
        ...
        vertexShader: document.getElementById( v_type + '.vert').text,
        fragmentShader: document.getElementById( f_type + '.frag').text,
        uniforms:{
    ...
    {
        var index0 = v0.x + v0.y*volume.resolution.x + v0.z*volume.resolution.x*volume.resolution.y;
        var index1 = v1.x + v1.y*volume.resolution.x + v1.z*volume.resolution.x*volume.resolution.y;
